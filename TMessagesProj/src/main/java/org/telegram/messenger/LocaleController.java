...
// import org.telegram.ui.RestrictedLanguagesSelectActivity; // Removed in Tony Chat
...
public class LocaleController {
        ...
        if (formatterDay == null) {
            synchronized (this) {
                if (formatterDay == null) {
                    ...
                    formatterDay = createFormatter(lang.toLowerCase().equals("ar") || lang.toLowerCase().equals("ko") ? locale : Locale.US, (is24HourFormat ? getStringInternal("formatterDay24H", R.string.formatterDay24H) : getStringInternal("formatterDay12H", R.string.formatterDay12H)).replace(":mm", false ? ":mm:ss" : ":mm"), (is24HourFormat ? "HH:mm" : "h:mm a").replace(":mm", false ? ":mm:ss" : ":mm"));
                }
            }
        }
        return formatterDay;
        ...
        if (formatterConstDay == null) {
            synchronized (this) {
                if (formatterConstDay == null) {
                    ...
                    lang = lang.toLowerCase();
                    formatterConstDay = createFormatter(lang.toLowerCase().equals("ar") || lang.toLowerCase().equals("ko") ? locale : Locale.US, (is24HourFormat ? "HH:mm" : "h:mm a").replace(":mm", false ? ":mm:ss" : ":mm"), (is24HourFormat ? "HH:mm" : "h:mm a").replace(":mm", false ? ":mm:ss" : ":mm"));
                }
            }
        }
    ...
    public static boolean usePersianCalendar = false; // need restart
    ...
    public int applyLanguage(final LocaleInfo localeInfo, boolean override, boolean init, boolean fromFile, boolean force, final int currentAccount, Runnable onDone) {
        ...
        try {
            ...
            if (!isLoadingRemote) {
                ...
                org.telegram.messenger.TranslateController.invalidateRestrictedLanguages();
                if (onDone != null) {
                    onDone.run();
                }
            }
    ...
    public static String formatDateOnline(long date, boolean[] madeShorter) {
        try {
            ...
            if (dateDay == day && year == dateYear) {
                ...
                if (usePersianCalendar) {
                    String format = getInstance().getFormatterDay().format(new Date(date));
    ...
    private void loadPrebuiltLocaleFile(LocaleInfo localeInfo) {
        ...
        // Skip asset loading - files not present
    }
